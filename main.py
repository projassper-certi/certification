import streamlit as st
from streamlit_gsheets import GSheetsConnection
import pandas as pd

# [핵심] 기존의 꼬인 기억(캐시)을 삭제합니다. (에러 방지용)
st.cache_data.clear()

st.set_page_config(layout="wide")
st.title("🏥 2025년도 인증 조사 평가 시스템")

# 1. 구글 시트 연결
conn = st.connection("gsheets", type=GSheetsConnection)

try:
    # ---------------------------------------------------------
    # 2. 데이터 불러오기 (여기가 핵심입니다!)
    # ---------------------------------------------------------
    
    # (1) 관리자 시트 읽기 (시트 이름: admin)
    # 혹시 또 에러나면 worksheet="admin" 대신 worksheet=1 로 바꿔보세요.
    df_admin = conn.read(worksheet=1, usecols=['이름', '기준번호'])
    
    # (2) 설문 데이터 시트 읽기 (시트 이름: 설문데이터)
    # skiprows=1 : 맨 윗줄(<붙임 1>...)을 건너뛰고 진짜 제목줄부터 읽습니다.
    df_main = conn.read(worksheet=0, skiprows=1)
    
    # (3) 데이터 다듬기
    # '기준번호'가 비어있는 잡다한 줄(행)은 삭제합니다.
    df_main = df_main.dropna(subset=['기준번호'])
    # 숫자 1.1과 문자 "1.1"이 헷갈리지 않게 전부 문자로 통일합니다.
    df_main['기준번호'] = df_main['기준번호'].astype(str)

except Exception as e:
    st.error(f"데이터 로딩 중 오류 발생! 에러 내용: {e}")
    st.stop()

# ---------------------------------------------------------
# 3. 사이드바 로그인 화면
# ---------------------------------------------------------
with st.sidebar:
    st.header("🔐 위원 로그인")
    st.write("관리자 시트에 등록된 이름을 입력하세요.")
    input_name = st.text_input("성함 입력", placeholder="예: 김철수")

# ---------------------------------------------------------
# 4. 메인 로직 (로그인 후 화면)
# ---------------------------------------------------------
if input_name:
    # 4-1. admin 시트에서 이름 찾기
    user_row = df_admin[df_admin['이름'] == input_name]
    
    if user_row.empty:
        st.error(f"⛔ '{input_name}' 위원님은 등록되지 않았습니다.")
        st.info("admin 시트에 이름이 정확히 있는지 확인해주세요.")
    else:
        st.success(f"👋 환영합니다, **{input_name}** 위원님!")
        
        # 4-2. 권한 가져오기 (콤마로 된 글자를 리스트로 변환)
        # 예: "1.1, 1.2" -> ['1.1', '1.2']
        permission_str = str(user_row.iloc[0]['기준번호'])
        target_ids = [x.strip() for x in permission_str.split(',')]
        
        # 4-3. 내 번호만 필터링하기
        my_data = df_main[df_main['기준번호'].isin(target_ids)]
        
        if my_data.empty:
            st.warning(f"배정된 기준번호({target_ids})에 해당하는 문항을 찾을 수 없습니다.")
        else:
            st.write(f"총 **{len(my_data)}개**의 평가 문항이 배정되었습니다.")
            
            # 4-4. 평가 화면 보여주기
            # (columns 리스트에 엑셀의 실제 열 이름들을 적어주면 더 깔끔하게 나옵니다)
            st.data_editor(
                my_data,
                hide_index=True,
                use_container_width=True,
                height=600,
                key="editor"
            )
            
            # (저장 버튼은 추후 구현)
            if st.button("평가 완료 및 저장"):
                st.toast("저장 기능은 다음 단계에서 만나요! 🚧")
                
else:
    # 로그인 전 안내 화면
    st.info("👈 왼쪽 사이드바에 성함을 입력해주세요.")
    st.markdown("---")
    st.write("### 📋 현재 등록된 위원 명단 (테스트용)")
    st.dataframe(df_admin)